    ...
    const fetchAchievements = async () => {
      try {
        const { data } = await axios.get(
          "https://admin.history-state.kz/api/v1/achievements-list/",
          {
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-7 mt-4">
        {displayedAchievements.map((achievement) => (
          ...
          >
            <img
              className="h-48 w-96"
              src={`https://admin.history-state.kz${achievement.image}`}
              alt=""
            />
      ...
      {selectedAchievement && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 overflow-y-auto">
          <div className="bg-[#E4E4E4] p-4 mb-4 w-3/4 h-full ">
            <div className="mx-auto overflow-y-auto p-4 scrollbar-thin scrollbar-thumb-[#2f457d] scrollbar-track-[#2f457d]">
