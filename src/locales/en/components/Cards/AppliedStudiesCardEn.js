    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://91.147.92.207:3000/api/v1/applied-research-list/"
        );
  ...
  useEffect(() => {
    const filtered = dataArray.filter(
      (item) =>
        ...
        item.target_en.toLowerCase().includes(searchTerm.toLowerCase()) ||
        item.tasks_en.toLowerCase().includes(searchTerm.toLowerCase()) ||
        ...
        <div className="flex flex-col">
          {displayedData.map((item, index) => (
            <div key={item.title_en}>
              ...
              >
                <h1 className="text-white text-xl font-bold">Topic:</h1>
              ...
              {openedItem === index && (
                <div className="bg-white p-2">
                    ...
                    <p
                      ...
                      dangerouslySetInnerHTML={{
                        __html: DOMPurify.sanitize(
                          item.tasks_en.replace(/\/media\//g, mediaUrl)
                        ),
                      }}
                    ></p>
                  </div>
                  ...
                  <p className="text-[#333] text-lg font-bold mt-2">
                    Implementation period: {item.implementation_period}
                  </p>
                </div>
              )}
...
export default AppliedStudiesCardEn;
