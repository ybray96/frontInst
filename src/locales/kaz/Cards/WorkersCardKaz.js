...
function WorkersCardKaz() {
  ...
  useEffect(() => {
    const apiUrl = "http://91.147.92.207:3000/api/v1/scientists-list/";
      ...
      <table className="w-full text-sm mt-4 border-collapse border border-gray-300">
        <thead className="text-xs uppercase bg-blue-500 text-white">
          <tr>
            ...
            <th className="px-6 py-3 border border-gray-300">БІЛІМ ДӘРЕЖЕСІ</th>
            ...
            .map((item, index) => (
              ...
              >
                <td className="p-4 flex items-center border border-gray-300">
                  ...
                  />
                  <span className="ml-2">{item.name_kk}</span>
                </td>
                <td className="px-6 py-4 border border-gray-300">
                  {item.degree_of_education_kk}
                </td>
                <td className="px-6 py-4 border border-gray-300">
                  {item.year_of_education_kk}
                </td>
                <td className="px-6 py-4 border border-gray-300">
                  {item.scientific_topics_kk}
                </td>
              </tr>
            ))}
        </tbody>
