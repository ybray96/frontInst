...
function RecomendationKZ() {
    ...
    const fetchRecommendations = async () => {
      try {
        const { data } = await axios.get(
          "http://127.0.0.1:8000/api/v1/recomendation-list/",
          {
  ...
  useEffect(() => {
    const filtered = recommendations.filter((item) =>
      item.file_name_kk.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    />
      ...
      <div className="mb-[145px] flex flex-col mx-auto max-w-screen-xl   px-4  ">
        <div className="text-2xl text-gray-800 whitespace-nowrap my-2 ">
          <h1 className="text-center">Ұсынымдар</h1>
        </div>
              ...
              <tbody>
                {displayedRecommendations.map((recommendation, index) => (
                  ...
                  >
                    ...
                    >
                      {recommendation.file_name_kk}
                    </th>
                    <td className="px-4 py-4 border-b border-gray-300">
                      <a
                        download
                        href={`http://127.0.0.1:8000/${recommendation.file}`}
...
export default RecomendationKZ;
